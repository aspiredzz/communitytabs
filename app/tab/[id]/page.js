    ...
    <AppShell activeTabId={id}>
      <div className="channel">
        <div className="channel-header">
          ...
          <div>
            <div className="channel-name">{tab ? tab.name : '...'}</div>
            {tab && tab.description && <div className="channel-desc">{tab.description}</div>}
          </div>
        </div>
        ...
        <div className="feed" ref={feedRef}>
          {items.length === 0 && (
            <div className="feed-empty">
              ...
              {tab && tab.description && <div className="feed-empty-desc">{tab.description}</div>}
            </div>
          )}
          {items.map((item, i) => (
            <div className="post" key={item.id} style={{ animationDelay: `${Math.min(i, 20) * 15}ms` }}>
